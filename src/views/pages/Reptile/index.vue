<template>
  <div class='reptileBox'>
    ...
    <p>示例：知乎排行榜 .Card .HotList-item HotList-itemTitle HotList-itemMetrics https://www.zhihu.com/billboard</p>
    <div class="w-1-1">
      <el-input type="text"
                placeholder="请输入网址"
      ...
      <el-form ref="ruleFormRef"
               ...
               label-width="120px"
               class="demo-dynamic">
        <el-form-item v-for="(item, index) in dynamicValidateForm.fieldList"
                      :key="item.id"
          ...
          <el-row class="full"
                  :gutter='10'>
            <el-col :span="15">
              <el-input :placeholder="index===0 ? '请输入父级容器id或class名称' : '请输入id或class名称' "
                        v-model="item.value"></el-input>
            </el-col>
            <el-col :span="4"
                    v-if="dynamicValidateForm.fieldList.length>1">
              <el-button type="danger"
                         @click="deleteHandler(item,index)">X</el-button>
            </el-col>
          </el-row>
...
const deleteHandler = (item, index) => {
  const { fieldList } = dynamicValidateForm
  if (fieldList.length <= 1) {//如果只有一个输入框则不可以删除
    return false
  }
